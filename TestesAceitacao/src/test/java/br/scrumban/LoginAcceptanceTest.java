...
public class LoginAcceptanceTest extends BaseAcceptanceTest {
	...
	@Test
	public void teste_login_com_falha_gera_entrada_na_tabela_falha_login() {
		Integer entradasEsperadasNaTabela = 1 ;
		String texto_mensagem_nao_logado = "Login failed";
		...
		boolean acheiTextoFalhaLogin = element.getText().equals(texto_mensagem_nao_logado);
		Assert.assertTrue(acheiTextoFalhaLogin);
		
		Integer resultadoEntradasFalhaLog = (Integer)emf.createEntityManager().createNativeQuery("select count(*) from FalhaLogin").getSingleResult();
		Assert.assertEquals(entradasEsperadasNaTabela,resultadoEntradasFalhaLog);
